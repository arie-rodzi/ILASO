...
def prepare_class_data(file_classes):
    ...
    if "ks" not in df.columns and "ks" in df.columns:
        df = df.rename(columns={"ks": "ks"})
    ...
    for col in ["campuran_group", "perincian", "ks_info", "pensyarah_asal", "lock_agihan"]:
...
def is_eligible_for_full_ks(lect_row):
    """Pensyarah aktif dan masuk selewat-lewatnya minggu 10 wajib diberi agihan."""
    if not bool(lect_row["active"]):
        return False
    return int(lect_row["minggu_mula_available"]) <= LATE_ENTRY_CUTOFF_WEEK


def can_assign_with_late_cover(lect_row, class_row):
    """
    Normal lecturer must cover the whole class.
    Late-entry lecturer up to week 10 may receive class with temporary cover for early weeks.
    """
    if can_cover_full_class(lect_row, class_row):
        return True
    if bool(lect_row["active"]) and int(lect_row["minggu_mula_available"]) <= LATE_ENTRY_CUTOFF_WEEK:
        return int(lect_row["minggu_mula_available"]) <= int(class_row["minggu_akhir_kelas"])
    return False
...
def solve_allocation(dfc, dfl, pref):
    ...
    - Semua kelas aktif mesti diagih.
    - Semua pensyarah aktif yang masuk minggu 1-10 mesti dapat sekurang-kurangnya 1 kelas.
    - Sasaran utama ialah 16 KS.
    - Julat adil ialah 15-17 KS.
    - 18 KS hanya emergency melalui slack, bukan default.
    - Preference digunakan selepas fairness.
    """
    ...
    ks = dfc.set_index("kelas_id")["ks"].astype(int).to_dict()
    ...
    eligible = {l: is_eligible_for_full_ks(lect_rows.loc[l]) for l in lecturers}

    prob = pl.LpProblem("ILASO_Fair_KS_Allocation", pl.LpMinimize)
    ...
    under_fair = pl.LpVariable.dicts("under_fair", lecturers, lowBound=0)
    over_fair = pl.LpVariable.dicts("over_fair", lecturers, lowBound=0)
    ...
    emergency_over = pl.LpVariable.dicts("emergency_over", lecturers, lowBound=0)
    zero_violation = pl.LpVariable.dicts("zero_violation", lecturers, 0, 1, cat="Binary")

    max_load_var = pl.LpVariable("max_load_eligible", lowBound=0)
    min_load_var = pl.LpVariable("min_load_eligible", lowBound=0)

    # 1 kelas = 1 pensyarah by default. Sharing only when explicitly allowed.
    for c in classes:
        if share_allowed[c] == "YA":
    ...
    # Inactive cannot teach; active must satisfy availability or late-cover rule.
    for c in classes:
        for l in lecturers:
            if not active[l]:
                prob += x[c][l] == 0
            elif not can_assign_with_late_cover(lect_rows.loc[l], class_rows.loc[c]):
                prob += x[c][l] == 0

    # Workload, fairness and emergency rule.
    for l in lecturers:
        total_load = pl.lpSum(ks[c] * x[c][l] for c in classes)

        if eligible[l]:
            # Wajib dapat sekurang-kurangnya satu kelas. Slack hanya untuk elak infeasible.
            prob += pl.lpSum(x[c][l] for c in classes) + zero_violation[l] >= 1

            # Fair range 15-17 KS, with emergency slack.
            prob += total_load + under_fair[l] >= FAIR_MIN_KS
            prob += total_load <= FAIR_MAX_KS + emergency_over[l]

            # Target 16 KS.
            prob += total_load + under_target[l] >= TARGET_KS
            prob += total_load - TARGET_KS <= over_target[l]

            # Range minimization among eligible lecturers.
            prob += total_load <= max_load_var
            prob += total_load >= min_load_var

            # Do not exceed individual max unless emergency slack is used.
            prob += total_load <= max(max_k[l], FAIR_MAX_KS) + emergency_over[l]
        else:
            # Cuti / tidak aktif / masuk selepas minggu 10: no forced allocation.
            prob += zero_violation[l] == 0
            prob += under_fair[l] == 0
            prob += over_fair[l] == 0
            prob += under_target[l] == 0
            prob += over_target[l] == 0
            prob += emergency_over[l] == 0
    ...
    # Subject limit: normally max 2 subjects; emergency 3 can be enabled manually later.
    for l in lecturers:
        prob += pl.lpSum(y[l][s] for s in subjects) <= MAX_SUBJECTS

    # Limit repeated classes under same subject.
    for l in lecturers:
        for s in subjects:
    ...
    preference_reward = pl.lpSum(
        ks[c] * get_pref_score(l, cls_subject[c], pref) * x[c][l]
        for c in classes for l in lecturers
    )

    fairness_penalty = pl.lpSum(
        under_fair[l] + over_target[l] + under_target[l]
        for l in lecturers if eligible[l]
    )

    zero_penalty = pl.lpSum(zero_violation[l] for l in lecturers if eligible[l])
    emergency_penalty = pl.lpSum(emergency_over[l] for l in lecturers if eligible[l])
    share_penalty = pl.lpSum(share[c] for c in classes)

    prob += (
        W_UNDER * fairness_penalty
        + W_BALANCE * pl.lpSum(over_target[l] + under_target[l] for l in lecturers if eligible[l])
        + W_RANGE * (max_load_var - min_load_var)
        + W_ZERO * zero_penalty
...
def build_outputs(dfc_active, df_closed, dfl, pref, assigned_df):
    ...
    if assigned_df.empty:
        ...
        for _, ar in assigned_df.iterrows():
            ...
            rows.append({
                ...
                "cover_sementara_diperlukan": "YA" if int(lrow["minggu_mula_available"]) > int(r["minggu_mula_kelas"]) else "TIDAK",
                "minggu_cover_sementara": (f"Minggu {int(r['minggu_mula_kelas'])}-{int(lrow['minggu_mula_available'])-1}" if int(lrow["minggu_mula_available"]) > int(r["minggu_mula_kelas"]) else "Tiada"),
                "penderma_subjek_asal": asal if asal and asal != lname else "Tiada / tidak diketahui",
    ...
    for _, lrow in dfl.iterrows():
        ...
        total_credit = int(tmp["ks"].sum()) if not tmp.empty else 0
        ...
        summary_rows.append({
            ...
            "aktif": active,
            "minimum_ks": int(lrow["min_kredit"]),
            "maksimum_ks": int(lrow["max_kredit"]),
            "minimum_efektif_ks": min_eff,
            "maksimum_efektif_ks": max_eff,
            "jumlah_ks_mengajar": total_credit,
...
st.markdown(
    '<div class="section-note">Minimum dan maksimum KS diambil terus daripada fail pensyarah. Semester fixed 14 minggu.</div>',
    unsafe_allow_html=True
)
...
if file_classes is None or file_lect is None:
    ...
    st.markdown(
        ...
        kod_kursus, kelas_baru, jam_ks<br><br>
    ...
    with manager_tabs[1]:
        ...
        with c2:
            new_credit = st.number_input("KS", 1, 10, 3, 1)
    ...
    with k3:
        metric_card("KS Aktif", int(df_active["ks"].sum()), "Jumlah KS perlu cover")
    ...
    if int(dfl["effective_max_kredit"].sum()) < int(df_active["ks"].sum()):
    ...
    if st.button("🚀 Run Allocation", use_container_width=True):
        ...
        with result_tabs[2]:
            ...
            st.caption("Sharing hanya dikira apabila kelas_id yang sama mempunyai lebih daripada seorang pensyarah. KS = Kredit Setara.")
        ...
        with result_tabs[3]:
            ...
            if px is not None and not df_summary.empty:
                fig = px.bar(
                    df_summary.sort_values("jumlah_ks_mengajar"),
                    x="jumlah_ks_mengajar",
                    y="pensyarah",
                    orientation="h",
                    text="jumlah_ks_mengajar",
                    title="Jumlah KS Mengikut Pensyarah"
                )
                fig.update_layout(height=680)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.dataframe(
                    df_summary[["pensyarah", "jumlah_ks_mengajar"]],
                    use_container_width=True
                )
